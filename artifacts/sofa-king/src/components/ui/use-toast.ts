import * as React from "react";
  ...
  export const reducer = (state: State, action: Action): State => {
    switch (action.type) {
      ...
      case "DISMISS_TOAST": { const { toastId } = action; if (toastId) { addToRemoveQueue(toastId); } else { state.toasts.forEach(t => addToRemoveQueue(t.id)); } return { ...state, toasts: state.toasts.map(t => !toastId || t.id === toastId ? { ...t, open: false } : t) }; }
