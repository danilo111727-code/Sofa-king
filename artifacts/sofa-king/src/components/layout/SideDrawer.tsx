import { X, Search, ChevronDown, ChevronRight, User, LogOut, UserPlus, Settings, Heart } from "lucide-react";
  ...
  function ClerkAuthSection({ onClose }: { onClose: () => void }) {
            ...
            <div className="mb-6">
              {hasClerk ? (
                <>
                  ...
                  <ClerkAuthSection onClose={onClose} />
                </>
              ) : (
                <Link href="/admin/login" onClick={onClose} data-testid="link-drawer-admin-login">
                  <button className="w-full flex items-center justify-center gap-2 py-3 bg-[#c9a96e] hover:bg-[#b8954f] text-[#1a1208] rounded-md text-sm font-semibold transition-colors">
